...
// Transcription function using OpenAI Whisper with native Node https
async function transcribeAudio(audioBuffer, filename) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Read the file as a buffer for the multipart form
    const fileBuffer = fs.readFileSync(tempFilePath);

    // Create multipart form data manually
    const boundary = '----formdata-multipart-boundary-' + randomBytes(16).toString('hex');
    ...
    formData += `Content-Type: audio/webm\r\n\r\n`;
      ...
      res.on('end', () => {
        try {
          if (res.statusCode !== 200) {
            console.error(`❌ OpenAI API returned status ${res.statusCode}:`, responseData);
            resolve("Your voice was recorded successfully. The transcription service is temporarily unavailable.");
            return;
          }
